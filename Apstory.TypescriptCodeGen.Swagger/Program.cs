        ...
        public static async Task RunCodeGen(string url, string version, string outputDirectory)
        {
            if (!string.IsNullOrEmpty(url) && !string.IsNullOrEmpty(version) && !string.IsNullOrEmpty(outputDirectory))
            {
                try
                {
                    ...
                    var tmg = new TypescriptModelGenerator($"{outputDirectory}\\models\\gen");
                    await tmg.Generate(se.GetClassModels());

                    var tasg = new TypescriptApiServiceGenerator($"{outputDirectory}\\services\\gen\\api\\v{version}", version);
